...
try {
    ...
    const blockedSites = (0, fs_1.readFileSync)(serve_1.TOP_LEVEL_DIR + "blockedSites.csv")
        .toString()
        .split(/\n/)
    ...
    (async () => {
        ...
        app.get("/overlay", (req, res) => {
            try {
                res.send((0, fs_1.readFileSync)(serve_1.TOP_LEVEL_DIR + "overlay.html").toString());
            }
            catch (e) {
                console.error(e);
                logger.error(e);
        ...
        app.get("/bin/index.js", (req, res) => {
            try {
                ...
                res.send((0, fs_1.readFileSync)(serve_1.TOP_LEVEL_DIR + "bin/index.js").toString());
            }
            catch (e) {
                console.error(e);
                logger.error(e);
        ...
        app.get("/", (0, express_limit_1.limit)({
            ...
            try {
                function sendError(e) {
                    ...
                    res.send((0, fs_1.readFileSync)(serve_1.TOP_LEVEL_DIR + "overlay.html")
                        .toString()
                        .replace(/\$value/, "") + `<h1>${e}</h1>`);
                }
                let url = req.query.url;
                let user = "user" in req.query ? req.query.user : undefined;
                ...
                if (isUrl(url)) {
                    if (!/^https?:\/\//.test(url))
                        url = "https://" + url;
                    let overlay = (0, fs_1.readFileSync)(serve_1.TOP_LEVEL_DIR + "overlay.html")
                        .toString()
                        .replace(/\$value/, (new RegExp(`^${searchEngine.replace(/\//g, "\\/").replace(/\?/g, "\\?")}`).test(url)
                        ? new URL(url).searchParams.get(searchParam) || ""
                        : url || "").replace(/"/g, '\\"'));
